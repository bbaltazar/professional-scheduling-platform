...
def compress_video(input_file, output_file, preset='medium', crf=23, scale=None, 
                   audio_bitrate='128k', format=None, two_pass=False):
    """
    ...
    try:
        if two_pass and format != 'webm':
            ...
            print("📊 Pass 1/2...")
            pass1_cmd = cmd[:-1] + ['-pass', '1', '-f', format, '/dev/null' if os.name != 'nt' else 'NUL']
...
Examples:
  ...
  python scripts/compress_video.py video.mov -o output.mp4 --two-pass
    ...
    parser.add_argument('-o', '--output', help='Output video file (default: input_compressed.mp4)')
    ...
    parser.add_argument('--crf', type=int, help='Custom CRF value (0-51, overrides quality preset)')
