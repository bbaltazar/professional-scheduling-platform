...
export function updateRecurringSchedulesCalendar() {
    ...
    const schedules = window.recurringSchedulesData || [];
    console.log(`Rendering calendar with ${schedules.length} schedules`);
    // Get time range from selectors
    const startHourElement = document.getElementById('scheduleViewTimeStart');
    const endHourElement = document.getElementById('scheduleViewTimeEnd');
        ...
        for (let dayIndex = 0; dayIndex < 7; dayIndex++) {
            ...
            const daySchedules = schedules.filter((schedule) => {
                if (schedule.recurrence_type === 'weekly') {
                    return schedule.days_of_week && schedule.days_of_week.includes(dayIndex);
                }
